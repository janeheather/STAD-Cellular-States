...
GSE26253_data=final
#saveRDS(GSE26253_data,'/public/workspace/liuqzh/gastric_cancer/bulk_data/GSE26253/GSE26253_data.rds')
GSE26253_data=readRDS('/public/workspace/liuqzh/gastric_cancer/bulk_data/GSE26253/GSE26253_data.rds')
genelist=read.csv("/public/workspace/liuqzh/gastric_cancer/scalop-master/MP_subtype_III.csv",sep = ",",header = T)
GSE26253_data<-GSE26253_data[rowSums(GSE26253_data)>0,]

GENE_list=list(genelist$MP_1)
ssGSEA_Score_MP1=gsva(as.matrix(GSE26253_data),GENE_list, method='ssgsea')

GENE_list=list(genelist$MP_2)
ssGSEA_Score_MP2=gsva(as.matrix(GSE26253_data),GENE_list, method='ssgsea')

GENE_list=list(genelist$MP_3)
ssGSEA_Score_MP3=gsva(as.matrix(GSE26253_data),GENE_list, method='ssgsea')
...
p <- ggplot(df, aes(x = group, y = Value)) +
	...
	labs(title = "GSE26253", x = "Group", y = "MP1 Score")  # 添加标签

# 计算并添加指定组间的 p 值
p <- p + stat_compare_means(comparisons = list(c("Recur", "Non_Recur")), method='wilcox.test',
                            label = "p.signif", label.y = c(2))

pdf('/public/workspace/liuqzh/gastric_cancer/bulk_data/GSE26253/GSE26253_MP1.pdf',height=6,width=4)
print(p)
dev.off()
...
p <- ggplot(df, aes(x = group, y = Value)) +
	...
	labs(title = "GSE26253", x = "Group", y = "MP2 Score")  # 添加标签

# 计算并添加指定组间的 p 值
p <- p + stat_compare_means(comparisons = list(c("Recur", "Non_Recur")), method='wilcox.test',
                            label = "p.signif", label.y = c(2))

pdf('/public/workspace/liuqzh/gastric_cancer/bulk_data/GSE26253/GSE26253_MP2.pdf',height=6,width=4)
print(p)
dev.off()
...
p <- ggplot(df, aes(x = group, y = Value)) +
	...
	labs(title = "GSE26253", x = "Group", y = "MP3 Score")  # 添加标签

# 计算并添加指定组间的 p 值
p <- p + stat_compare_means(comparisons = list(c("Recur", "Non_Recur")), method='t.test',
                            label = "p.signif", label.y = c(2))

pdf('/public/workspace/liuqzh/gastric_cancer/bulk_data/GSE26253/GSE26253_MP3.pdf',height=6,width=4)
print(p)
dev.off()
