...
###P13_EGC
aix_single_data=aix[grep("P13_EGC",rownames(aix)),]
pdf('/public/workspace/liuqzh/gastric_cancer/Tumor/scrabble_score_Subtype_III_plot_P13_EGC.pdf',width=9,height=8,useDingbats=F)
colors=c("MP_1"="#D1832C","MP_2"="#5B93D4","MP_3"="#A97598")
p1<-ggplot(aix_single_data, aes(x=x_name, y=y_name)) + 
...
###Tu15
aix_single_data=aix[grep("Tu15",rownames(aix)),]
...
###Tu3
aix_single_data=aix[grep("Tu3",rownames(aix)),]
pdf('/public/workspace/liuqzh/gastric_cancer/Tumor/scrabble_score_Subtype_III_plot_Tu3.pdf',width=9,height=8,useDingbats=F)
colors=c("MP_1"="#D1832C","MP_2"="#5B93D4","MP_3"="#A97598")
p1<-ggplot(aix_single_data, aes(x=x_name, y=y_name)) + 
	geom_point(aes(color = Subtype), size = 0.8) +
	stat_density_2d(aes(fill = after_stat(level)), geom = "polygon", color = NA, alpha = 0.1) +# 添加密度趋势
...
###P02_AGC
aix_single_data=aix[grep("P02_AGC",rownames(aix)),]
pdf('/public/workspace/liuqzh/gastric_cancer/Tumor/scrabble_score_Subtype_III_plot_P02_AGC.pdf',width=9,height=8,useDingbats=F)
colors=c("MP_1"="#D1832C","MP_2"="#5B93D4","MP_3"="#A97598")
p1<-ggplot(aix_single_data, aes(x=x_name, y=y_name)) + 
	geom_point(aes(color = Subtype), size = 0.8) +
	stat_density_2d(aes(fill = after_stat(level)), geom = "polygon", color = NA, alpha = 0.1) +# 添加密度趋势
